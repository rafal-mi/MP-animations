...
fun Animation14() {
    ...
    val animationSpecTween = tween<Float>(
        durationMillis = duration,
        easing = LinearEasing,
    )
    ...
    val buttonScaleXTween by animateFloatAsState(
        targetValue = if(expanded) 1.0f else 0.0f,
        animationSpec = animationSpecTween
    )
    val buttonScaleYTween by animateFloatAsState(
        targetValue = if(expanded) 1.0f else 0.0f,
        animationSpec = animationSpecTween
    )
    ...
    val buttonWHeight = animateDpAsState(
        targetValue = if(expanded) {
